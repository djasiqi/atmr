    ...
    with app.app_context():
        ...
        rows_wrong_owner = (
            db.session.query(
                ...
                Booking.company_id,
                BookingTransfer.id,
                BookingTransfer.owner_company_id,
            )
        ...
        print(
            "1) Bookings où company_id ≠ owner_company_id du transfert (ACCEPTED/COMPLETED)"
        )
        if not rows_wrong_owner:
            ...
            for (
                ...
                print(
                    f"   booking_id={booking_id} company_id={company_id} transfer_id={transfer_id} owner_company_id={owner_company_id}"
                )
        ...
        if not rows_missing_executor:
            ...
            for transfer_id, booking_id in rows_missing_executor:
                print(f"   transfer_id={transfer_id} booking_id={booking_id}")
