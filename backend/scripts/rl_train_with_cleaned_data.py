...
def convert_cleaned_data_to_rl(
    ...
    print("=" * 80)
    print(f"📂 Source : {cleaned_data_file}")
    ...
    print(f"📊 {len(training_data)} enregistrements chargés")
    ...
    print(f"📅 {len(dispatches_by_date)} jours trouvés")
    ...
    print(f"✅ {len(rl_dispatches)} dispatches créés")

    # Statistiques
    if rl_dispatches:
        total_bookings = sum(d['num_bookings'] for d in rl_dispatches)
        total_drivers = sum(d['num_drivers'] for d in rl_dispatches)
        avg_bookings = total_bookings / len(rl_dispatches)
        avg_drivers = total_drivers / len(rl_dispatches)

        print("📊 Statistiques :")
        print(f"   - Total bookings : {total_bookings}")
        print(f"   - Total drivers : {total_drivers}")
        print(f"   - Moyenne bookings/dispatch : {avg_bookings:.1f}")
        print(f"   - Moyenne drivers/dispatch : {avg_drivers:.1f}")
...
def train_with_cleaned_data():
    ...
    train_offline(
        ...
        num_episodes=15000,  # Plus d'épisodes pour plus de données
