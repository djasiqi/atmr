...
def migrate_users(dry_run: bool = False, batch_size: int = 100):
    ...
    while offset < total_count:
        ...
        logger.debug("Chunk %d-%d traité: %d mis à jour, %d total migré", chunk_start, offset + len(users), chunk_updated, migrated)
        offset += chunk_size
...
def migrate_clients(dry_run: bool = False, batch_size: int = 100):
    ...
    while offset < total_count:
        ...
        logger.debug("Chunk %d-%d traité: %d mis à jour, %d total migré", chunk_start, offset + len(clients), chunk_updated, migrated)
        offset += chunk_size
...
def main():
    ...
    parser.add_argument(
        "--users-only",
        action="store_true",
        help="Migrer uniquement les utilisateurs"
    )
    parser.add_argument(
        "--clients-only",
        action="store_true",
        help="Migrer uniquement les clients"
    )
