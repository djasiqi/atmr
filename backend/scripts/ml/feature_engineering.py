...
def normalize_features(
    ...
    if len(continuous_cols) > 0:
        ...
        for _col in continuous_cols[:5]:  # Afficher les 5 premières
            print("   - {col}")
        if len(continuous_cols) > 5:
            print("   ... et {len(continuous_cols) - 5} autres")
...
def split_train_test(
    ...
    print("✅ Train set : {len(train_df)} échantillons ({(1-test_size)*100")
    print("✅ Test set  : {len(test_df)} échantillons ({test_size*100")

    # Vérifier distribution du target
    if target in df.columns:
        print("\n📊 Distribution du target :")
        print("   Train - Moyenne : {train_df[target].mean()")
        print("   Test  - Moyenne : {test_df[target].mean()")
        print("   Différence      : {abs(train_df[target].mean() - test_df[target].mean())")
...
def main():
    ...
    try:
        ...
        print("✅ {len(df_original)} échantillons × {len(df_original.columns)} features")
        ...
        print("✅ Dataset complet : {full_path}")
        ...
        with Path(scalers_path, "w").open() as f:
            json.dump(scalers_data, f, indent=2)
        print("✅ Scalers : {scalers_path}")
        ...
        print("="*70)
        print("\n📊 Résumé :")
        print("   Features  : {len(df_original.columns)} → {len(df.columns)} (+{len(df.columns) - len(df_original.columns)})")
        print("   Train set : {len(train_normalized)} ({(1-args.test_size)*100")
        print("   Test set  : {len(test_normalized)} ({args.test_size*100")
        print("="*70)
