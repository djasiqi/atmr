...
def generate_visualizations(
    ...
    if 'date' in comparisons_df.columns:
        ...
        plt.savefig(agreement_file, dpi=300)
        plt.close()
        generated_files.append(str(agreement_file))
        print(f"✅ Graphique sauvegardé: {agreement_file}")
    ...
    if action_dist:
        ...
        plt.savefig(actions_file, dpi=300)
        plt.close()
        generated_files.append(str(actions_file))
        print(f"✅ Graphique sauvegardé: {actions_file}")
    ...
    if 'confidence' in comparisons_df.columns:
        ...
        plt.savefig(conf_file, dpi=300)
        plt.close()
        generated_files.append(str(conf_file))
        print(f"✅ Graphique sauvegardé: {conf_file}")
...
def main():
    ...
    print(f"   Prédictions : {len(predictions_df)}")
    print(f"   Comparaisons: {len(comparisons_df)}")
    print()
    ...
    if agreement_analysis:
        print("\n📈 TAUX D'ACCORD:")
        print(f"   Global    : {agreement_analysis['overall_agreement_rate']:.1%}")
        print(f"   Total     : {agreement_analysis['total_comparisons']} comparaisons")
        print(f"   Accords   : {agreement_analysis['agreements']}")

        if 'high_confidence_agreement' in agreement_analysis:
            print(f"   High conf : {agreement_analysis['high_confidence_agreement']:.1%}")
            print(f"   Low conf  : {agreement_analysis['low_confidence_agreement']:.1%}")

    action_analysis = analyze_action_distribution(predictions_df, comparisons_df)

    if action_analysis:
        print("\n📊 DISTRIBUTION DES ACTIONS:")
        print(f"   DQN     : {action_analysis['dqn']['assign_rate']:.1%} assigns")
        print(f"   Actuel  : {action_analysis['actual']['assign_rate']:.1%} assigns")

    # Visualisations
    print("\n📊 Génération des visualisations...")
    viz_files = generate_visualizations(predictions_df, comparisons_df, args.output_dir)
    print(f"   {len(viz_files)} graphiques générés")
