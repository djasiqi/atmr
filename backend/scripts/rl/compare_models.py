...
def evaluate_config(config: dict, episodes: int = 200, name: str = "Config") -> dict:
    ...
    print("     Learning rate : {config.get('learning_rate', 0.0001)")
    print("     Gamma         : {config.get('gamma', 0.99)")
    print("     Batch size    : {config.get('batch_size', 64)}")
    print("     Epsilon decay : {config.get('epsilon_decay', 0.995)")
    ...
    for episode in range(episodes):
        ...
        while not done and steps < 100:
            ...
            if len(agent.memory) >= agent.batch_size:
                agent.learn()  # Utiliser learn() au lieu de train_step()
        ...
        # Progress
        if (episode + 1) % 50 == 0:
            np.mean(training_rewards[-50:])
            print("      Episode {episode + 1}/{episodes} - Avg reward: {avg_recent")
    ...
    print("\n   📈 Résultats:")
    print("      Reward moyen : {results['mean_reward']")
    print("      Reward médian: {results['median_reward']")
    print("      Range        : [{results['min_reward']")
    print("      Assignments  : {results['mean_assignments']")
    print("      Late pickups : {results['mean_late_pickups']")
    print("      Distance     : {results['mean_distance']")
...
def main():
    ...
    print("Config optimale: {args.optimal_config}")
    print("=" * 70)
    ...
    try:
        ...
        if Path(args.optimal_config).exists():
            with Path(args.optimal_config, encoding="utf-8").open() as f:
        ...
        if optimal_results:
            ...
            print("-" * 70)
            print("{'Reward moyen':<20} {baseline_results['mean_reward']:>12.1f} {optimal_results['mean_reward']:>12.1f} {improvement:>13.1f}%")
            print("{'Distance (km)':<20} {baseline_results['mean_distance']:>12.1f} {optimal_results['mean_distance']:>12.1f} {distance_improvement:>13.1f}%")
            print("{'Late pickups':<20} {baseline_results['mean_late_pickups']:>12.1f} {optimal_results['mean_late_pickups']:>12.1f} {late_improvement:>13.1f}")
            print("{'Assignments':<20} {baseline_results['mean_assignments']:>12.1f} {optimal_results['mean_assignments']:>12.1f} {'N/A':>15}")
            ...
            if improvement > 5:
                ...
                print("   L'optimisation a amélioré les performances de {improvement")
            elif improvement > 0:
        ...
        with Path(args.output, "w", encoding="utf-8").open() as f:
