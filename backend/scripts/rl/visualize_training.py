...
def plot_training_curves(metrics_file: str, output_dir: str = "data/rl/visualizations"):
    ...
    print(f"   ✅ {episodes_count} épisodes chargés")
    ...
    ax.axvline(np.mean(episode_rewards), color='r', linestyle='--', linewidth=2, label=f'Moyenne: {np.mean(episode_rewards):.1f}')
    ax.axvline(np.median(episode_rewards), color='g', linestyle='--', linewidth=2, label=f'Médiane: {np.median(episode_rewards):.1f}')
    ...
    print("\n📊 STATISTIQUES")
    print(f"   Reward moyen     : {np.mean(episode_rewards):.1f}")
    print(f"   Reward médian    : {np.median(episode_rewards):.1f}")
    print(f"   Reward min/max   : [{np.min(episode_rewards):.1f}, {np.max(episode_rewards):.1f}]")
    print(f"   Écart-type       : {np.std(episode_rewards):.1f}")
    print(f"   Epsilon final    : {epsilon_values[-1]:.4f}")
