...
def collect_dispatch_data(days_back: int = 90) -> pd.DataFrame:
    ...
    """
    print(f"📊 Collecte des données des {days_back} derniers jours...")
    ...
    print(f"✅ {len(assignments)} assignments trouvés")
    ...
    for assign in assignments:
        try:
            ...
            print(f"⚠️  Erreur sur assignment {assign.id}: {e}")
            continue
    ...
    print(f"  Lignes retirées: {initial_count - len(df)}")
...
def main():
    ...
    with app.app_context():
        ...
        print(f"  Distance moyenne: {stats['avg_distance_km']:.2f} km")
        print(f"  Durée moyenne: {stats['avg_duration_min']:.1f} min")
        print(f"  Taux de retard: {stats['late_rate']:.1%}")
        if stats['avg_rating']:
            print(f"  Note moyenne: {stats['avg_rating']:.2f}/5")

        print(f"\n  Distance P50: {stats['distance_p50']:.2f} km")
        print(f"  Distance P90: {stats['distance_p90']:.2f} km")
        print(f"  Distance P99: {stats['distance_p99']:.2f} km")
        ...
        print(f"💾 Politique baseline sauvegardée: {baseline_path}")

        print("\n✅ Collecte terminée!")
        print(f"   {len(df)} assignments prêts pour l'entraînement")
        print("="*60)
