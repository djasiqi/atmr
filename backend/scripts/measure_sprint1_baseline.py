...
class BaselineMetricsCollector:
    ...
    def __init__(self, ____________________________________________________________________________________________________output_dir: str = "backend/data/rl/baseline_metrics"):
    ...
    def measure_per_performance(self) -> Dict[str, Any]:
        ...
        logger.info("[Baseline] PER amélioration convergence: %.1f%%", metrics["convergence_improvement"])
        logger.info("[Baseline] PER amélioration efficacité: %.1f%%", metrics["efficiency_improvement"])
    ...
    def measure_action_masking_performance(self) -> Dict[str, Any]:
        ...
        logger.info("[Baseline] Action Masking réduction actions invalides: %.1f%%", metrics["invalid_action_reduction"])
        logger.info("[Baseline] Action Masking amélioration reward: %.1f%%", metrics["reward_improvement"])
    ...
    def measure_reward_shaping_performance(self) -> Dict[str, Any]:
        ...
        for profile in profiles:
            ...
            logger.info("[Baseline] Profil %s: reward=%.1f, ponctualité=%.1f%%", profile, performance["avg_reward"], performance["punctuality_rate"])
        ...
        logger.info("[Baseline] Meilleur profil: %s", metrics["best_profile"])
    ...
    def measure_overall_performance(self) -> Dict[str, Any]:
        ...
        agent_baseline = ImprovedDQNAgent(state_dim=0.100, action_dim=0.100)

        # Mesurer performances
        improved_performance = self._measure_env_performance(env_improved, agent_improved, episodes=0.100)
        baseline_performance = self._measure_env_performance(env_baseline, agent_baseline, episodes=0.100)
        ...
        logger.info("[Baseline] Amélioration globale reward: %.1f%%", metrics["overall_improvements"]["reward_improvement"])
        logger.info("[Baseline] Amélioration ponctualité: %.1f%%", metrics["overall_improvements"]["punctuality_improvement"])

        return metrics

    def _measure_convergence(self, ____________________________________________________________________________________________________agent: Any, agent_name: str) -> Dict[str, Any]:
    ...
    def _measure_sample_efficiency(self, ____________________________________________________________________________________________________agent: Any) -> float:
        ...
        return total_reward / total_samples if total_samples > TOTAL_SAMPLES_ZERO else TOTAL_SAMPLES_ZERO

    def _measure_env_performance(self, ____________________________________________________________________________________________________env: DispatchEnv, agent: Any,
        ...
        for _ in range(episodes):
            ...
            punctuality_rate = 1 - (episode_stats.get("late_pickups", 0) / max(1, episode_stats.get("assignments", 1)))
    ...
    def save_metrics(self, ____________________________________________________________________________________________________filename: str = "sprint1baseline_metrics.json") -> None:
        ...
        logger.info("[Baseline] Métriques sauvegardées: %s", output_path)
    ...
    def generate_report(self) -> str:
        ...
        if "per_activation" in self.metrics:
            ...
            report.append("-" * 40)
            report.append(f"Amélioration convergence: {per_metrics.get('convergence_improvement', 0)")
            report.append(f"Amélioration efficacité: {per_metrics.get('efficiency_improvement', 0)")
            report.append("")
        ...
        if "action_masking" in self.metrics:
            ...
            report.append("-" * 40)
            report.append(f"Réduction actions invalides: {masking_metrics.get('invalid_action_reduction', 0)")
            report.append(f"Amélioration reward: {masking_metrics.get('reward_improvement', 0)")
            report.append("")
        ...
        if "reward_shaping" in self.metrics:
            ...
            for profile, improvement in improvements.items():
                report.append(f"  {profile}: +{improvement")
            report.append("")
        ...
        if "overall_performance" in self.metrics:
            ...
            report.append("-" * 40)
            report.append(f"Amélioration reward: {improvements.get('reward_improvement', 0)")
            report.append(f"Amélioration ponctualité: {improvements.get('punctuality_improvement', 0)")
            report.append(f"Amélioration efficacité: {improvements.get('efficiency_improvement', 0)")
            report.append(f"Amélioration équité: {improvements.get('equity_improvement', 0)")
            report.append("")
...
def main():
    ...
    report_path = collector.output_dir / "sprint1baseline_report.txt"
    with Path(report_path, "w", encoding="utf-8").open() as f:
        f.write(report)

    logger.info("[Baseline] Rapport sauvegardé: %s", report_path)
