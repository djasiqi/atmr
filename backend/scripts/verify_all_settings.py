...
def verify_all_settings(company_id: int = 1):
    ...
    print(f"   Proximité:")
    print(f"     DB: {proximity_db} → Settings: {proximity_settings} {'✅' if proximity_db == proximity_settings or (proximity_db is None and proximity_settings == 0.2) else '❌'}")
    print(f"   Équilibre charge:")
    print(f"     DB: {driver_load_db} → Settings: {driver_load_settings} {'✅' if driver_load_db == driver_load_settings or (driver_load_db is None and driver_load_settings == 0.7) else '❌'}")
    print(f"   Priorité:")
    print(f"     DB: {priority_db} → Settings: {priority_settings} {'✅' if priority_db == priority_settings or (priority_db is None and priority_settings == 0.06) else '❌'}")
    ...
    print(f"   Temps limite (secondes):")
    print(f"     DB: {time_limit_db} → Settings: {time_limit_settings} {'✅' if time_limit_db == time_limit_settings or (time_limit_db is None and time_limit_settings == 60) else '❌'}")
    print(f"   Courses max par chauffeur:")
    print(f"     DB: {max_bookings_db} → Settings: {max_bookings_settings} {'✅' if max_bookings_db == max_bookings_settings or (max_bookings_db is None and max_bookings_settings == 6) else '❌'}")
    print(f"   Pénalité non-assigné:")
    print(f"     DB: {unassigned_penalty_db} → Settings: {unassigned_penalty_settings} {'✅' if unassigned_penalty_db == unassigned_penalty_settings or (unassigned_penalty_db is None and unassigned_penalty_settings == 10000) else '❌'}")
    ...
    print(f"   Pickup (minutes):")
    print(f"     DB: {pickup_db} → Settings: {pickup_settings} {'✅' if pickup_db == pickup_settings or (pickup_db is None and pickup_settings == 5) else '❌'}")
    print(f"   Dropoff (minutes):")
    print(f"     DB: {dropoff_db} → Settings: {dropoff_settings} {'✅' if dropoff_db == dropoff_settings or (dropoff_db is None and dropoff_settings == 10) else '❌'}")
    print(f"   Marge transition (minutes):")
    print(f"     DB: {margin_db} → Settings: {margin_settings} {'✅' if margin_db == margin_settings or (margin_db is None and margin_settings == 15) else '❌'}")
    ...
    print(f"   Activer le regroupement:")
    print(f"     DB: {pooling_enabled_db} → Settings: {pooling_enabled_settings} {'✅' if pooling_enabled_db == pooling_enabled_settings or (pooling_enabled_db is None and pooling_enabled_settings == True) else '❌'}")
    ...
    print(f"   Activer l'équité:")
    print(f"     DB: {fairness_enabled_db} → Settings: {fairness_enabled_settings} {'✅' if fairness_enabled_db == fairness_enabled_settings or (fairness_enabled_db is None and fairness_enabled_settings == True) else '❌'}")
    print(f"   Fenêtre d'équité (jours):")
    print(f"     DB: {fairness_window_db} → Settings: {fairness_window_settings} {'✅' if fairness_window_db == fairness_window_settings or (fairness_window_db is None and fairness_window_settings == 7) else '❌'}")
    print(f"   Poids équité (0-1):")
    print(f"     DB: {fairness_weight_db} → Settings: {fairness_weight_settings} {'✅' if fairness_weight_db == fairness_weight_settings or (fairness_weight_db is None and fairness_weight_settings == 0.3) else '❌'}")
    ...
    print(f"   Autoriser chauffeurs d'urgence:")
    print(f"     DB: {allow_emergency_db} → Settings: {allow_emergency_settings} {'✅' if allow_emergency_db == allow_emergency_settings or (allow_emergency_db is None and allow_emergency_settings == True) else '❌'}")
    print(f"   Pénalité d'utilisation (0-1000):")
    print(f"     DB: {emergency_penalty_db} → Settings: {emergency_penalty_settings} {'✅' if emergency_penalty_db == emergency_penalty_settings or (emergency_penalty_db is None and emergency_penalty_settings == 900.0) else '❌'}")
    ...
    if preferred_driver_db and preferred_driver_db in driver_load_multipliers_db:
        ...
        print(f"   Multiplicateur de charge:")
    ...
    print("📊 RÉSUMÉ")
    print("=" * 80)
    print(f"✅ Paramètres vérifiés depuis: autonomous_config.dispatch_overrides")
    print(f"✅ Settings calculés via: for_company(company)")
