...
def autocomplete_address(
    ...
    *,
    ...
    if not GOOGLE_API_KEY:
        raise GooglePlacesError("Clé API Google Maps non configurée (GOOGLE_MAPS_API_KEY)")
    ...
    try:
        ...
        if data.get("status") not in ("OK", "ZERO_RESULTS"):
            ...
            app_logger.warning(f"⚠️ Google Places Autocomplete: {error_msg}")
            return []
        ...
        app_logger.error(f"❌ Erreur Google Places Autocomplete: {e}")
        raise GooglePlacesError(f"Erreur lors de l'autocomplete: {e}") from e
...
def get_place_details(
    ...
    *,
    ...
    """
    if not GOOGLE_API_KEY:
        raise GooglePlacesError("Clé API Google Maps non configurée")
    ...
    try:
        ...
        if data.get("status") != "OK":
            ...
            raise GooglePlacesError(f"Erreur Place Details: {error_msg}")
        ...
        app_logger.error(f"❌ Erreur Google Place Details: {e}")
        raise GooglePlacesError(f"Erreur lors de la récupération des détails: {e}") from e
...
def geocode_address_google(
    ...
    *,
    ...
    if not address:
        raise ValueError("Adresse vide ou invalide")
    ...
    try:
        ...
        if data.get("status") == "ZERO_RESULTS":
            app_logger.warning(f"⚠️ Aucun résultat de géocodage pour: {address}")
            return None

        if data.get("status") != "OK":
            error_msg = data.get("error_message", data.get("status"))
            app_logger.warning(f"⚠️ Google Geocoding: {error_msg}")
            return None
        ...
        app_logger.error(f"❌ Erreur Google Geocoding API: {e}")
        raise GooglePlacesError(f"Erreur lors du géocodage: {e}") from e
...
def geocode_and_validate(
    ...
    *,
    ...
    if location_type not in ("ROOFTOP", "RANGE_INTERPOLATED", "GEOMETRIC_CENTER"):
        app_logger.warning(
            f"⚠️ Géolocalisation imprécise ({location_type}) pour: {address}"
        )
    ...
    if require_street_number and not components.get("street_number"):
        app_logger.warning(f"⚠️ Adresse sans numéro de rue: {address}")
        return None
