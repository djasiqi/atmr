...
class AgentOrchestrator:
    ...
    def _tick(self) -> None:
        ...
        should_act = (
            new_bookings or 
            drivers_became_unavailable or 
            unassigned_increased or 
            preferred_driver_changed or
            (is_first_tick and unassigned_jobs and current_preferred_driver_id)
        )
        if should_act:
            if preferred_driver_changed:
                logger.info(
    ...
    def _handle_progressive_decision(
        ...
        if preferred_driver_changed and not unassigned_jobs:
            ...
            if job_dates:
                ...
                if plan and plan.get("plan"):
                    success = self._apply_plan_with_validation(plan.get("plan", []), "full", now, preferred_driver_changed=True)
                    if not success:
                        logger.warning("[AgentOrchestrator] ⚠️ Plan initial rejeté, ré-optimisation avec contraintes renforcées")
        ...
        new_unassigned_jobs = [
            ...
            if j.get("job_id") in new_bookings
        ]
    ...
    def _apply_plan_with_validation(
        self, plan: list[Dict[str, Any]], strategy: str, now: datetime, max_retries: int = 3, preferred_driver_changed: bool = False  # noqa: ARG002
    ) -> bool:
        ...
        for step in plan:
            ...
            if booking:
                ...
                if existing_assignment:
                    logger.debug(
                        "[AgentOrchestrator] ⏭️ Job %s déjà assigné, skip",
                        job_id
                    )
                    continue
...
def stop_agent_for_company(company_id: int) -> None:
    ...
    with _agents_lock:
        ...
        if agent:
            agent.stop()
            logger.info(
                "[AgentOrchestrator] Stopped agent for company %s", company_id
            )
