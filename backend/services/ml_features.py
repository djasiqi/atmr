...
def extract_base_features(booking: Any, driver: Any) -> dict[str, float]:
    ...
    try:
        ...
        dropoff_lat = float(dropoff_lat_val) if dropoff_lat_val is not None else 0
    ...
    is_medical = 1 if medical_facility_val else 0
    is_urgent_val = getattr(booking, "is_urgent", False)
    is_urgent = 1 if is_urgent_val else 0
    is_round_trip_val = getattr(booking, "is_round_trip", False)
    is_round_trip = 1 if is_round_trip_val else 0
...
def create_aggregated_features(features: dict[str, float]) -> dict[str, float]:
    ...
    if driver_bookings < DRIVER_BOOKINGS_THRESHOLD:
        aggregated["driver_experience_level"] = 0  # novice
        aggregated["delay_by_driver_exp"] = 8.2
    elif driver_bookings < DRIVER_BOOKINGS_THRESHOLD:
        aggregated["driver_experience_level"] = 1  # intermédiaire
        aggregated["delay_by_driver_exp"] = 6.1
    else:
        aggregated["driver_experience_level"] = 2  # expert
        aggregated["delay_by_driver_exp"] = 4.3

    # Catégorie de distance
    distance = features["distance_km"]
    if distance < DISTANCE_THRESHOLD:
        aggregated["distance_category"] = 0
    elif distance < DISTANCE_THRESHOLD:
        aggregated["distance_category"] = 1
    elif distance < DISTANCE_THRESHOLD:
        aggregated["distance_category"] = 2
    else:
        aggregated["distance_category"] = 3

    # Niveau de trafic
    traffic = features["traffic_density"]
    if traffic < TRAFFIC_LOW_THRESHOLD:
        aggregated["traffic_level"] = 0
    elif traffic < TRAFFIC_MEDIUM_THRESHOLD:
        aggregated["traffic_level"] = 1
    else:
        aggregated["traffic_level"] = 2
