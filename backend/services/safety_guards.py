...
class SafetyGuards:
    ...
    def _perform_safety_checks(self, metrics: Dict[str, Any]) -> Dict[str, bool]:
        ...
        driver_utilization = metrics["avg_driver_load"] / max(1, metrics["max_driver_load"]) if metrics["max_driver_load"] > 0 else 1.0
    ...
    def _log_violations(self, checks: Dict[str, bool], metrics: Dict[str, Any]) -> None:
        ...
        logger.warning(
            "[SafetyGuards] ⚠️ Violations détectées: %s",
            ", ".join(violations)
        )
