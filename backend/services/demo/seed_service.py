...
def _upsert_user(
    *,
    ...
    user.set_password(get_demo_default_password())  # nosemgrep python.django.security.audit.unvalidated-password.unvalidated-password - mot de passe demo par defaut (seed isole).
    db.session.add(user)
    db.session.flush()
    return user
