...
class MLMonitoringService:
    ...
    def log_prediction(
        ...
        try:
            ...
            prediction = MLPrediction(
                ...
                feature_flag_enabled=stats["ml_enabled"],
                traffic_percentage=stats["ml_traffic_percentage"],
            )
            ...
            logger.debug(
                f"[MLMonitoring] Logged prediction for booking {booking_id}: "
            ...
            logger.error(f"[MLMonitoring] Failed to log prediction: {e}")
            db.session.rollback()
            raise
    ...
    def update_actual_delay(booking_id: int, actual_delay: float) -> None:
        ...
        try:
            ...
            if prediction:
                ...
                logger.info(
                    f"[MLMonitoring] Updated actual delay for booking {booking_id}: "
                    f"predicted={prediction.predicted_delay_minutes:.2f}, "
                    f"actual={actual_delay:.2f}, "
                    f"error={prediction.prediction_error:.2f}"
                )
            else:
                logger.warning(
                    f"[MLMonitoring] No prediction found for booking {booking_id}"
                )
                
        except Exception as e:
            logger.error(f"[MLMonitoring] Failed to update actual delay: {e}")
            db.session.rollback()
    ...
    def get_metrics(hours: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"[MLMonitoring] Failed to calculate metrics: {e}")
            return {"error": str(e)}
    ...
    def get_daily_metrics(days: int = 7) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"[MLMonitoring] Failed to calculate daily metrics: {e}")
            return []
    ...
    def get_recent_predictions(limit: int = 100) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"[MLMonitoring] Failed to get recent predictions: {e}")
            return []
    ...
    def detect_anomalies(threshold_mae: float = 5.0) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"[MLMonitoring] Failed to detect anomalies: {e}")
            return []
    ...
    def get_summary() -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"[MLMonitoring] Failed to get summary: {e}")
            return {"error": str(e)}
