...
class ProblemState:
    ...
    def from_problem(cls, problem: Dict[str, Any], drivers: List[Any]) -> ProblemState:
        ...
        if previous_busy or previous_times or previous_load:
            logger.debug(
                f"[ProblemState] État récupéré: "
                f"{len(previous_busy)} busy_until, "
                f"{len(previous_times)} scheduled_times, "
                f"{len(previous_load)} proposed_load"
            )
    ...
    def has_time_conflict(
        ...
        for existing_time in existing_times:
            if abs(time_min - existing_time) < min_gap_minutes:
                logger.debug(
                    f"[ProblemState] Conflit détecté pour driver #{driver_id}: "
                    f"course à {existing_time}min vs nouvelle à {time_min}min "
                    f"(écart: {abs(time_min - existing_time)}min < {min_gap_minutes}min)"
                )
                return True
    ...
    def assign_booking(
        ...
        logger.debug(
            f"[ProblemState] Driver #{driver_id} assigné: "
            f"start={start_time_min}, end={end_time_min}, "
            f"busy_until={self.busy_until[driver_id]}, "
            f"load={self.proposed_load[driver_id]}"
        )
    ...
    def reset_driver(self, driver_id: int) -> None:
        ...
        logger.debug(f"[ProblemState] Driver #{driver_id} réinitialisé")
