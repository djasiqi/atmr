...
class WarmStartGainTracker:
    ...
    def record_comparison(
        ...
        if recent:
            ...
            # Médian
            self.median_gain_pct = gains_pct_sorted[n // 2] if n % 2 == 1 else (gains_pct_sorted[n // 2 - 1] + gains_pct_sorted[n // 2]) / 2
            ...
            logger.info(
                ...
                self.median_gain_pct, self.avg_gain_pct, self.should_alert, n
            )
            
            if self.should_alert:
                logger.warning(
                    "[B5] ⚠️ Warm-start gain trop faible: %.1f%% < %.1f%% (sur 7j)",
                    self.median_gain_pct, MIN_GAIN_THRESHOLD_PCT
                )
...
def measure_warm_start_gain(
    ...
    if not (TARGET_SIZE_MIN <= size <= TARGET_SIZE_MAX):
        logger.debug(
            ...
            size, TARGET_SIZE_MIN, TARGET_SIZE_MAX
        )
        return {
            "skipped": True,
