...
def check_slo_breach(
    ...
    return {
        ...
        "slo_target": {
            "category": "small" if n_bookings < SMALL_BATCH_THRESHOLD else ("medium" if n_bookings < LARGE_BATCH_THRESHOLD else "large"),
...
class SLOBreachTracker:
    ...
    def get_breach_summary(self, current_time: float) -> Dict[str, Any]:
        ...
        severity = "critical" if len(recent) >= CRITICAL_BREACH_COUNT else ("warning" if len(recent) >= WARNING_BREACH_COUNT else "info")
