...
HIGH_DELAY_RATE_THRESHOLD = 0.5
MODERATE_DELAY_RATE_THRESHOLD = 0.3
TOTAL_ASSIGNMENTS_ZERO = 0
DELAY_RATE_ZERO = 0
AVERAGE_DELAY_THRESHOLD = 15
DRIVERS_COUNT_ZERO = 0
RATIO_THRESHOLD = 5
DELAYED_COUNT_ZERO = 0
...
class DelayPredictor:
    ...
    def _calculate_severity(self, delay_minutes: int) -> str:
        ...
        if abs_delay < self.delay_thresholds["low"]:
            return "low"
        if abs_delay < self.delay_thresholds["medium"]:
            return "medium"
        if abs_delay < self.delay_thresholds["high"]:
            return "high"
        return "critical"
    ...
    def _generate_recommendations(self, analysis: DelayAnalysis, problem: Dict[str, Any]) -> List[str]:
        ...
        if analysis.total_assignments > TOTAL_ASSIGNMENTS_ZERO:
        ...
        if drivers_count > DRIVERS_COUNT_ZERO:
...
def predict_delays_for_dispatch(
    ...
    Usage:
        ...
        if analysis.delayed_count > DELAYED_COUNT_ZERO:
            print("Attention: {analysis.delayed_count} retards prévus!")
    """
