        ...
        if not user:
            ...
            if user.role == UserRole.driver:
                if not getattr(
                    user,
                    "driver",
                        None) or user.driver.company_id != company_id:
                ...
                if not getattr(
                    user,
                    "company",
                        None) or user.company.id != company_id:
            ...
            if result is None:
                ...
                try:
                    ...
                    if result is None:
                        ...
                        company_name = company.name if company and getattr(
                            company, "name", None) else "Entreprise"
                        ...
                        for m in messages:
                            ...
                            if not base:
                                ...
                                base = {
                                    "id": m.id, "company_id": m.company_id, "sender_id": getattr(
                                        m, "sender_id", None), "receiver_id": getattr(
                                        m, "receiver_id", None), "sender_role": getattr(
                                        m, "sender_role", None), "content": getattr(
                                        m, "content", None), "timestamp": m.timestamp.isoformat() if getattr(
                                        m, "timestamp", None) else None, }
                                # enrichir noms
                                base["sender_name"] = (
                                    company_name if getattr(
                                        m,
                                        "sender_role",
                                        None) in (
                                        "COMPANY",
