...
dispatch_health_ns = Namespace(
    "company_dispatch_health",
    description="Monitoring de santé du dispatch")
...
health_response = dispatch_health_ns.model(
    "DispatchHealthResponse",
    {
        ...
        "avg_assignment_rate": fields.Float(
            description="Taux d'assignation moyen"),
        "avg_run_time_sec": fields.Float(
            description="Temps d'exécution moyen en secondes"),
        "osrm_availability": fields.Float(
            description="Disponibilité OSRM (0-1)"),
        "failed_runs": fields.Integer(
            description="Nombre de runs échoués"),
        "unassigned_reasons": fields.Raw(
            description="Raisons de non-assignation"),
        "performance_trend": fields.String(
            description="Tendance de performance"),
    })
        ...
        for run in successful_runs:
            ...
            if summary:
                ...
                if "osrm_availability" in summary:
                    osrm_availability_scores.append(
                ...
                if "unassigned_reasons" in summary:
                    for _booking_id, reasons in summary["unassigned_reasons"].items(
                        ...
                        for reason in reasons:
                            unassigned_reasons[reason] = unassigned_reasons.get(
                                reason, 0) + 1

        # Calculer les moyennes
        avg_assignment_rate = sum(
            assignment_rates) / len(assignment_rates) if assignment_rates else 0.0
        avg_run_time_sec = sum(run_times) / \
            len(run_times) if run_times else 0.0
        osrm_availability = sum(osrm_availability_scores) / len(
            osrm_availability_scores) if osrm_availability_scores else 1.0

        # Déterminer la tendance de performance
        if len(assignment_rates) >= MIN_RATES_FOR_TREND:
            recent_avg = sum(
                assignment_rates[-RECENT_RATES_COUNT:]) / min(RECENT_RATES_COUNT, len(assignment_rates))
            older_avg = sum(assignment_rates[:-RECENT_RATES_COUNT]) / max(
                1, len(assignment_rates) - RECENT_RATES_COUNT) if len(assignment_rates) > MIN_COMPARISON_RATES else recent_avg
    ...
    def get(self):
        ...
        for run in runs:
            ...
            if run.status == "FAILED":
                ...
                if isinstance(summary, dict):
                    ...
                    if "run_time_sec" in summary:
                        daily_data[day_key]["run_times"].append(
                            summary["run_time_sec"])
                    if "osrm_availability" in summary:
        ...
        while current_date <= end_date:
            ...
            if day_key in daily_data:
                ...
                avg_assignment_rate = sum(data["assignment_rates"]) / len(
                    data["assignment_rates"]) if data["assignment_rates"] else 0.0
                avg_run_time = sum(
                    data["run_times"]) / len(data["run_times"]) if data["run_times"] else 0.0
                avg_osrm = sum(
                    data["osrm_scores"]) / len(data["osrm_scores"]) if data["osrm_scores"] else 1.0
