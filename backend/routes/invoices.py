...
class InvoicesList(Resource):
    ...
    def get(self, company_id):
        ...
        items, total_count, stats = list_company_invoices_unified(
        ...
        links = {
            "first": f"/api/invoices?page=1&per_page={per_page}",
        }
        if page < pages:
            links["next"] = f"/api/invoices?page={page + 1}&per_page={per_page}"
        if page > 1:
            links["prev"] = f"/api/invoices?page={page - 1}&per_page={per_page}"
        if pages > 0:
            links["last"] = f"/api/invoices?page={pages}&per_page={per_page}"
