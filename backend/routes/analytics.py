...
analytics_ns = Namespace(
    "analytics",
    description="Analytics et métriques de dispatch")
...
class AnalyticsDashboard(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            logger.debug(
                "[Analytics] Company: %s, err: %s, code: %s",
                company,
                err,
                code)
            ...
            if err or company is None:
                msg = (
                    err or {}).get("error") if isinstance(
            ...
            if start_str and end_str:
                try:
                    ...
                    return {
                        "success": False, "error": "Invalid date format. Use YYYY-MM-DD"}, 400
            else:
            ...
            logger.debug(
                "[Analytics] Generating insights for company %s",
                company.id)
            insights = generate_insights(company.id, analytics)
            analytics["insights"] = insights

            logger.info(
                "[Analytics] Returning analytics data: %s days", len(
                    analytics.get(
                        "trends", [])))
            ...
            return {"success": False,
                    "error": f"Failed to fetch analytics: {e!s}"}, 500
...
class AnalyticsInsights(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            return {"success": False,
                    "error": f"Failed to generate insights: {e!s}"}, 500
...
class WeeklySummary(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            return {"success": False,
                    "error": f"Failed to fetch weekly summary: {e!s}"}, 500
...
class ExportAnalytics(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            return {"success": False,
                    "error": f"Failed to export analytics: {e!s}"}, 500
