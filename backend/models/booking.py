...
class Booking(db.Model):
    ...
    billed_to_company = relationship(
        "Company", foreign_keys=[billed_to_company_id])
    ...
    @staticmethod
    def _enforce_billing_exclusive(
            _mapper, _connection, target: Booking) -> None:
        btype = (
            _as_str(
                getattr(
                    target,
                    "billed_to_type",
                    None)) or "patient").strip().lower()
        if btype == "patient":
