...
class ABTestResult(db.Model):
    ...
    test_timestamp = db.Column(db.DateTime, nullable=False, default=datetime.utcnow, index=True)
    ...
    def __repr__(self) -> str:
        return f"<ABTestResult {self.id} Booking:{self.booking_id} ML:{self.ml_delay_minutes:.2f} Heuristic:{self.heuristic_delay_minutes:.2f}>"
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "actual_delay_minutes": round(self.actual_delay_minutes, 2) if self.actual_delay_minutes is not None else None,
