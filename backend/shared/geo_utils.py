...
def validate_coordinates(lat: float, lon: float) -> bool:
    ...
    Note:
        - Vérifie que lat est entre -90 et 90
        - Vérifie que lon est entre -180 et 180
        - Vérifie que les valeurs sont des nombres (float/int)
    """
    # Vérifier le type (nécessaire pour validation runtime)
    if not isinstance(lat, (int, float)) or not isinstance(lon, (int, float)):  # pyright: ignore[reportUnnecessaryIsInstance]
        return False
    ...
    # Vérifier que ce ne sont pas des valeurs NaN ou Infinity
    import math

    if math.isnan(lat) or math.isnan(lon):
        return False
    return not (math.isinf(lat) or math.isinf(lon))


# ✅ Refactoring: Classe GeoValidator pour centraliser validation coordonnées
class GeoValidator:
    """Validateur centralisé pour coordonnées géographiques.

    Factorise la validation des coordonnées GPS utilisée dans plusieurs modules
    ...
    @staticmethod
    def validate(lat: float, lon: float) -> None:
        """Valide les coordonnées et lève une exception si invalides.
