...
class DriverProfileUpdateSchema(Schema):
    ...
    # Statut
    status = fields.Str(
        validate=validate.OneOf(["disponible", "hors service"], error="status doit être: 'disponible' ou 'hors service'")
    )
    ...
    employment_end_date = fields.Date(
        validate=validate.Regexp(ISO8601_DATE_REGEX),
        allow_none=True
    )
    ...
    medical_valid_until = fields.Date(
        validate=validate.Regexp(ISO8601_DATE_REGEX),
        allow_none=True
    )
    ...
    trainings = fields.List(
        fields.Dict(),
        validate=validate.Length(max=50, error="Maximum 50 formations")
    )
