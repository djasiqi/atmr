...
class PaymentCreateSchema(Schema):
    ...
    amount = fields.Float(required=True, validate=validate.Range(min=0.01, error="Le montant doit être > 0"))
    method = fields.Str(required=True, validate=validate.Length(min=1, max=50, error="Méthode de paiement requise"))
...
class PaymentStatusUpdateSchema(Schema):
    ...
    status = fields.Str(
        ...
        validate=validate.OneOf(
            ["pending", "completed", "failed"], error="Statut invalide. Valeurs possibles: pending, completed, failed"
        ),
    )
