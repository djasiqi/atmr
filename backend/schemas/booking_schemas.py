...
class BookingCreateSchema(Schema):
    ...
    amount = fields.Float(required=True, validate=validate.Range(min=0, error="amount doit être >= 0"))
...
class BookingUpdateSchema(Schema):
    ...
    status = fields.Str(validate=validate.OneOf(["pending", "confirmed", "in_progress", "completed", "cancelled"]))
...
class BookingListSchema(Schema):
    ...
    page = fields.Int(load_default=1, validate=validate.Range(min=1, error="page doit être >= 1"))
    per_page = fields.Int(
        load_default=100, validate=validate.Range(min=1, max=500, error="per_page doit être entre 1 et 500")
    )
    status = fields.Str(
        load_default=None,
        validate=validate.OneOf(["pending", "confirmed", "in_progress", "completed", "cancelled"]),
        allow_none=True,
    )
    from_date = fields.Str(
        load_default=None,
        validate=validate.Regexp(ISO8601_DATE_REGEX, error="from_date doit être au format YYYY-MM-DD"),
        allow_none=True,
    )
    to_date = fields.Str(
        load_default=None,
        validate=validate.Regexp(ISO8601_DATE_REGEX, error="to_date doit être au format YYYY-MM-DD"),
        allow_none=True,
    )
