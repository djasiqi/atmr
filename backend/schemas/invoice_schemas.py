...
class BillingSettingsUpdateSchema(Schema):
    ...
    overdue_fee = fields.Float(validate=validate.Range(min=0, error="overdue_fee doit être >= 0"))
    reminder1fee = fields.Float(validate=validate.Range(min=0, error="reminder1fee doit être >= 0"))
    reminder2fee = fields.Float(validate=validate.Range(min=0, error="reminder2fee doit être >= 0"))
    reminder3fee = fields.Float(validate=validate.Range(min=0, error="reminder3fee doit être >= 0"))
    ...
    iban = fields.Str(
        validate=validate.Regexp(r"^[A-Z]{2}[0-9]{2}[A-Z0-9]{1,30}$", error="IBAN invalide"), allow_none=True
    )
    qr_iban = fields.Str(
        validate=validate.Regexp(r"^[A-Z]{2}[0-9]{2}[A-Z0-9]{1,30}$", error="QR IBAN invalide"), allow_none=True
    )
    ...
    vat_rate = fields.Float(
        allow_none=True, validate=validate.Range(min=0, max=100, error="vat_rate doit être entre 0 et 100")
    )
...
class InvoiceGenerateSchema(Schema):
    ...
    period_year = fields.Int(
        required=True, validate=validate.Range(min=2000, max=2100, error="Année invalide (2000-2100)")
    )
    period_month = fields.Int(required=True, validate=validate.Range(min=1, max=12, error="Mois invalide (1-12)"))
    ...
    overrides = fields.Dict(
        keys=fields.Str(), values=fields.Dict(keys=fields.Str(), values=fields.Raw()), allow_none=True
    )
