...
class PricingSimulateBookingSchema(Schema):
    ...
    pickup_admin_token = fields.Str(
        required=False, allow_none=True, validate=validate.Length(max=64)
    )
    dropoff_admin_token = fields.Str(
        required=False, allow_none=True, validate=validate.Length(max=64)
    )
    pickup_lat = fields.Float(
        required=False, allow_none=True, validate=validate.Range(min=-90, max=90)
    )
    pickup_lng = fields.Float(
        required=False, allow_none=True, validate=validate.Range(min=-180, max=180)
    )
    ...
    pickup_zip = fields.Str(
        required=False, allow_none=True, validate=validate.Length(max=16)
    )
    dropoff_zip = fields.Str(
        required=False, allow_none=True, validate=validate.Length(max=16)
    )
...
class PricingSimulateRequestSchema(Schema):
    pricing_profile_version_id = fields.Int(
...
class PricingZoneMatrixSettingsSchema(Schema):
    ...
    default_same_zone_price = fields.Float(
        required=False, allow_none=True, validate=validate.Range(min=0)
    )
