...
class MedicalEstablishmentQuerySchema(Schema):
    ...
    q = fields.Str(
        validate=validate.Length(
            max=200, error="Query trop longue (max 200 caractères)"
        ),
        allow_none=True,
        load_default=None,
    )
    limit = fields.Int(
        validate=validate.Range(min=1, max=25, error="limit doit être entre 1 et 25"),
        load_default=8,
    )
...
class MedicalServiceQuerySchema(Schema):
    ...
    establishment_id = fields.Int(
        required=True,
        validate=validate.Range(min=1, error="establishment_id doit être > 0"),
    )
    q = fields.Str(
        validate=validate.Length(
            max=200, error="Query trop longue (max 200 caractères)"
