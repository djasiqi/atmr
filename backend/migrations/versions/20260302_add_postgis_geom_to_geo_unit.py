...
def upgrade():
    ...
    op.execute("ALTER TABLE geo_unit ADD COLUMN IF NOT EXISTS geom geometry(MultiPolygon, 4326)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_geo_unit_geom ON geo_unit USING GIST (geom)")
    op.execute(
