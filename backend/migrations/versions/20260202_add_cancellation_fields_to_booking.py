...
def upgrade():
    with op.batch_alter_table("booking", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column("cancellation_reason_text", sa.Text(), nullable=True)
        )
        batch_op.add_column(
            sa.Column("is_cancellation_billable", sa.Boolean(), nullable=True)
        )
        batch_op.add_column(
            sa.Column("cancellation_display_label", sa.String(120), nullable=True)
        )
