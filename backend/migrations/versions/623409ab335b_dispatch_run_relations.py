...
def upgrade():
    ...
    with op.batch_alter_table("assignment", schema=None) as batch_op:
        batch_op.alter_column(
            "dispatch_run_id", existing_type=sa.INTEGER(), nullable=True
        )
    ...
    op.execute(
        "UPDATE dispatch_run SET status = UPPER(status) WHERE status IS NOT NULL;"
    )
...
def downgrade():
    ...
    op.execute(
        "UPDATE dispatch_run SET status = LOWER(status) WHERE status IS NOT NULL;"
    )
    ...
    with op.batch_alter_table("assignment", schema=None) as batch_op:
        ...
        batch_op.alter_column(
            "dispatch_run_id", existing_type=sa.INTEGER(), nullable=False
        )
