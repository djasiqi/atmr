...
class TestDispatchSafetyIntegration:
    ...
    def test_safe_dispatch_with_rl_optimization(self, mock_company, mock_drivers, mock_bookings, mock_assignments):
        ...
        with patch("services.safety_guards.get_safety_guards") as mock_get_guards:
            ...
            with patch("services.unified_dispatch.rl_optimizer.RLDispatchOptimizer") as mock_optimizer_class:
                ...
                # Simuler le dispatch
                result = self._simulate_dispatch(mock_company, mock_drivers, mock_bookings, mock_assignments)
        ...
        with patch("services.safety_guards.get_safety_guards") as mock_get_guards:
            ...
            safety_guards.check_dispatch_result.return_value = (
                False,
                {
                    "is_safe": False,
                    "violation_count": 3,
                    "checks": {"max_delay_ok": False, "completion_rate_ok": False, "driver_load_ok": False},
                },
            )
            mock_get_guards.return_value = safety_guards
    ...
    def test_rl_optimizer_safety_check(self, mock_drivers, mock_bookings, mock_assignments):
        ...
        with patch("services.safety_guards.get_safety_guards") as mock_get_guards:
            ...
            with (
                ...
                result = optimizer.optimize_assignments(mock_assignments, mock_bookings, mock_drivers)
    ...
    def test_notification_service_integration(self, mock_company, mock_drivers, mock_bookings, mock_assignments):
        ...
        with patch("services.safety_guards.get_safety_guards") as mock_get_guards:
            ...
            with patch("services.notification_service.NotificationService") as mock_notification_class:
                ...
                # Simuler le dispatch
                _result = self._simulate_dispatch(mock_company, mock_drivers, mock_bookings, mock_assignments)
    ...
    def test_performance_under_high_load(self, mock_company, mock_drivers, mock_bookings, mock_assignments):
        ...
        with patch("services.safety_guards.get_safety_guards") as mock_get_guards:
            ...
            for _ in range(100):
                self._simulate_dispatch(mock_company, mock_drivers, mock_bookings, mock_assignments)
    ...
    def _simulate_dispatch(self, company, drivers, bookings, assignments):
        ...
        try:
            ...
            is_safe, safety_result = safety_guards.check_dispatch_result(dispatch_metrics, rl_metadata)
