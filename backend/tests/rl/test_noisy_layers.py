...
class TestNoisyQNetwork:
    ...
    def test_noise_reset_network(self):
        ...
        for i, layer in enumerate(network.layers):
            assert not torch.equal(layer.weight_epsilon, initial_noises[i]["weight"])
            assert not torch.equal(layer.bias_epsilon, initial_noises[i]["bias"])
...
class TestFactoryFunctions:
    ...
    def test_create_noisy_network(self):
        ...
        # Test type invalide
        with pytest.raises(ValueError, match="Type de réseau non supporté"):
            create_noisy_network(network_type="invalid", state_size=10, action_size=5)
    ...
    def test_create_q_network(self):
        ...
        noisy_dueling_network = create_q_network(
            network_type="noisy_dueling", state_dim=10, action_dim=5, use_noisy=True
        )
