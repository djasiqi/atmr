...
def test_driver_id_from_payload_sets_token() -> None:
    driver = _Driver(id=1)
    uc = SaveDriverPushTokenUseCase(
        user_repo=_UserRepo(None), driver_repo=_DriverRepo(driver)
    )
    res = uc.execute(payload={"token": "x" * 10, "driverId": 1}, jwt_identity=None)
    assert res.status_code == 200
    assert driver.push_token == "x" * 10
    assert res.should_commit is True


def test_fallback_jwt_resolves_driver() -> None:
    driver = _Driver(id=7)
    user = SimpleNamespace(id=123)
    uc = SaveDriverPushTokenUseCase(
        user_repo=_UserRepo(user), driver_repo=_DriverRepo(driver)
    )
    res = uc.execute(payload={"token": "x" * 10}, jwt_identity="public_id")
    assert res.status_code == 200
    assert driver.push_token == "x" * 10
