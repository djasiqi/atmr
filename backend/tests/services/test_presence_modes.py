...
def test_compute_location_status_for_mission_live_thresholds():
    ...
    assert (
        compute_location_status(mode="mission_live", last_seen_seconds=50) == "recent"
    )
    assert (
        compute_location_status(mode="mission_live", last_seen_seconds=150) == "stale"
    )
    assert (
        compute_location_status(mode="mission_live", last_seen_seconds=301) == "offline"
    )
