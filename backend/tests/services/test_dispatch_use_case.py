...
def test_dispatch_use_case_prepare_params() -> None:
    uc = DispatchUseCase(
        ...
        validate_assignments_fn=lambda _a, **_kw: {"valid": True, "errors": [], "warnings": []},
    )

    params = uc.prepare_dispatch_params(
        validated_data={"for_date": "2025-01-15", "regular_first": False, "allow_emergency": True, "overrides": {"x": 1}},
        company_id=123,
        effective_mode="auto",
    )
...
def test_dispatch_use_case_execute_dispatch_sync_calls_engine_and_validation() -> None:
    ...
    result, validation_info = uc.execute_dispatch_sync({"company_id": 1, "for_date": "2025-01-15"})
...
def test_dispatch_use_case_validate_and_normalize_rejects_unknown_override_keys() -> None:
    uc = DispatchUseCase(
        get_bookings_for_day_fn=lambda _cid, _d: [],
        getenv_fn=lambda _k, _d: _d,
        engine_run_fn=lambda **_kw: {},
    ...
    validated, error_response, status = uc.validate_and_normalize_request(
        DispatchRunRequestCommand(company_id=1, body={"for_date": "2025-01-15", "overrides": {"unknown_key": {"a": 1}}})
    )
