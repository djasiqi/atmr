...
class TestDisasterScenarios:
    ...
    def test_osrm_down_10_min(self, app_context: Flask, dispatch_scenario, reset_chaos):
        ...
        try:
            ...
            haversine_fallback_detected = (
                "haversine fallback" in log_output.lower()
                or "fallback_matrix" in log_output.lower()
                or "All attempts failed, using haversine fallback" in log_output
            )

            logger.info(
                "[D3] Résultat dispatch avec OSRM down: %s assignments",
    ...
    def test_db_read_only(self, app_context: Flask, authenticated_client, reset_chaos, sample_company, sample_client):
        ...
        assert (
            ...
            or "writes are temporarily disabled" in error_message
        ), f"Message d'erreur devrait mentionner read-only, reçu: {response_data}"

        logger.info(
            "[D3] ✅ Écriture bloquée correctement avec message: %s",
    ...
    def test_network_flaky(self, app_context: Flask, reset_chaos):
        ...
        logger.info(
            "[D3] Chaos activé: latence %sms, error_rate %.0f%%",
            NETWORK_FLAKY_LATENCY_MS,
            NETWORK_FLAKY_ERROR_RATE * 100,
        )
        ...
        try:
            ...
            fallback_detected = (
                "haversine fallback" in log_output.lower()
                or "fallback_matrix" in log_output.lower()
                or "All attempts failed" in log_output
            )
    ...
    def test_combined_disaster(self, app_context: Flask, dispatch_scenario, reset_chaos):
        ...
        db_load_futures = [db_load_executor.submit(db_load_worker) for _ in range(5)]
