...
def test_should_skip_dedup_emits_pipeline_log(caplog):
    ...
    with (
        patch(
            "services.notifications.dedup_throttle._get_redis", return_value=mock_redis
        ),
        patch("services.notifications.metrics.record_dedup_hit"),
    ...
    payload = json.loads(
        caplog.records[-1].message.split("[notification_pipeline] ", 1)[1]
    )
...
def test_should_skip_throttle_emits_pipeline_log(caplog):
    ...
    with (
        ...
        patch("services.notifications.metrics.record_throttle_block"),
        caplog.at_level("INFO"),
    ):
        assert (
            should_skip_throttle("driver", 7, "scope", window_s=60, max_per_window=3)
            is True
        )
