...
class TestCORSConfiguration:
    ...
    def test_cors_headers_present(self, client):
        ...
        # ]
        # Commenté car non utilisé (CORS peut ne pas être configuré)
        ...
        # Si CORS n'est pas configuré, aucun header ne sera présent
        # (c'est OK pour API interne)
        # Vérifier qu'au moins un header CORS est présent si CORS est configuré
        # La variable est préfixée par _ car elle est vérifiée visuellement
        # mais pas utilisée dans un assert
        # (car CORS peut ne pas être configuré,
        # ce qui est acceptable pour une API interne)
...
class TestCSRFTokenHandling:
    ...
    def test_csrf_token_not_required_for_jwt_auth(self, client, auth_headers):
        """Test que les requêtes authentifiées avec JWT
        n'ont pas besoin de token CSRF."""
...
class TestCSRFDocumentation:
    ...
    def test_document_why_csrf_not_needed(self):
        ...
        # 3. CORS contrôle l'accès cross-origin
        # 4. Les requêtes doivent inclure le token JWT dans le header,
        # pas dans un cookie
        #
