...
class TestAddToBlacklist:
    ...
    def test_add_to_blacklist_success(self, mock_decode, mock_redis):
        ...
        # Mock Redis - mock_redis est directement le client Redis
...
class TestIsTokenBlacklisted:
    ...
    def test_is_token_blacklisted_true(self, mock_redis):
        ...
        assert result is True
        mock_redis.exists.assert_called_once_with(f"{BLACKLIST_PREFIX}test-jti-123")

    @patch("security.token_blacklist.redis_client")
    def test_is_token_blacklisted_false(self, mock_redis):
        """Test vérification token non blacklisté (retourne False)."""
        # Mock Redis - mock_redis est directement le client Redis
        # S'assurer que le mock est truthy (pas None)
    ...
    @patch("security.token_blacklist.redis_client", None)
    def test_is_token_blacklisted_redis_unavailable(self):
    ...
    def test_is_token_blacklisted_with_jwt_token(self, mock_decode, mock_redis):
        ...
        assert result is True
        mock_redis.exists.assert_called_once_with(f"{BLACKLIST_PREFIX}test-jti-456")
    ...
    def test_is_token_blacklisted_without_jti(self, mock_decode, mock_redis):
        ...
        assert result is True
        mock_redis.exists.assert_called_once_with(f"{BLACKLIST_PREFIX}{token_hash}")
    ...
    def test_is_token_blacklisted_decode_error(self, mock_decode, mock_redis):
        ...
        # Mock Redis - mock_redis est directement le client Redis
        # Pas besoin de configurer exists car l'exception sera levée avant
...
class TestRevokeToken:
    ...
    @patch("security.token_blacklist.redis_client", None)
    @patch("security.token_blacklist.get_jwt")
    def test_revoke_token_redis_unavailable(self, mock_get_jwt):
        ...
        # Devrait retourner False, pas lever d'exception
        result = revoke_token()
        assert result is False
