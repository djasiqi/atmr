...
def train_offline(
    ...
    save_path: str = "data/rl/models/dispatch_optimized_v1.pth",
        ...
        while not done and step < num_bookings:
            ...
            if action > 0:  # Action != "wait"
                ...
                if booking_idx < num_bookings:
                    ...
                    # Calculer distance de la course
                    booking = bookings[booking_idx]
                    if (
                        booking.get("pickup_lat")
                        and booking.get("dropoff_lat")
                    ):
                        try:
                    ...
                    # Pénalité légère pour distance (secondaire)
                    distance_penalty = -0.5 * dist if "dist" in locals() else 0
...
def _create_state_from_dispatch(env: DispatchEnv, dispatch: dict) -> np.ndarray:
    ...
    for i in range(num_drivers):
        if i < len(env.drivers):
            ...
            # Position initiale = premier booking (simplifié)
            if env.bookings:
                first = env.bookings[0]
                env.drivers[i]["lat"] = first.get("pickup_lat", 46.2044)
                env.drivers[i]["lon"] = first.get("pickup_lon", 6.1432)
