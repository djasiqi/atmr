...
describe("fleetMarkerIcons", () => {
  it("génère un SVG 42×42 avec initiales et anneau blanc", () => {
    const svg = buildFleetCircleMarkerSvgMarkup("#4ade80", 42, 1, { label: "AB" });
  ...
  it("produit une data URL encodée pour cercle", () => {
    const url = makeFleetCircleMarkerDataUrl("#00796B", 42, 1, { label: "CD" });
    expect(url.startsWith("data:image/svg+xml;charset=UTF-8,")).toBe(true);
    expect(decodeURIComponent(url.split(",")[1] ?? "")).toContain("#00796B");
  });

  it("initiales limitées à 2 caractères", () => {
    expect(resolveDriverMarkerInitials("Jean Dupont")).toBe("JE");
  });
