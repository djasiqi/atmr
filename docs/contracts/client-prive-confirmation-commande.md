...
✅ **Implémenté** : `resolve_portal_terms_status` (`backend/services/legal/portal_terms_status.py`) est la seule source. Le catalogue `current_portal_terms()` désigne l’unique version courante par document. `requires_reacceptance` est décidé à la publication et figé sur `legal_document_version` ; le service ne compare pas les textes. `is_current` n’est pas une colonne mutable (la table refuse les UPDATE) : la version courante est le catalogue. `published_at` est `legal_document_version.created_at`.
...
✅ **Implémenté** : une modification client PORTAL (`PUT /api/v1/bookings/<id>`) et une annulation client PORTAL (`DELETE /api/v1/bookings/<id>` et `DELETE /api/v1/clients/me/bookings/<id>`) ajoutent `BOOKING_MODIFIED` ou `BOOKING_CANCELLED` dans la même transaction, avec `sequence_number` suivant et verrou de la réservation. `BOOKING_CREATED` n’est pas réécrit. Les conditions référencées restent celles de la création : une réacceptation ultérieure n’est pas recopiée, et `TERMS_REACCEPTANCE_REQUIRED` ne bloque ni la modification ni l’annulation. Le motif d’annulation reste vide, les routes n’en demandent pas. `UpdatePendingBookingUseCase` n’applique pas l’aller-retour, l’heure de retour ni le fauteuil : ces champs du schéma, s’ils sont seuls dans le payload, ne changent pas la commande et ne créent pas d’événement. L’heure de retour déjà figée est recopiée depuis `BOOKING_CREATED`. Une mutation par une entreprise ou un admin ne produit pas d’événement `actor_type=client`. Aucun événement historique n’est inventé.
