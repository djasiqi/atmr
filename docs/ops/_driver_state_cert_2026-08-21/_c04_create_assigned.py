...
with app.app_context():
    ...
    # Toujours company du chauffeur (pas celle du client fallback) — sinon PUT status → 403
    b.company_id = company_id
    b.executing_company_id = company_id
    b.client_id = int(client["client_id"])
    b.driver_id = DRIVER_ID
    b.customer_name = "CANARY-C04-TASKDEF"
    ...
    db.session.add(b)
    db.session.flush()
    # ARRIVED-SOT-1B : invariant Assignment obligatoire
    ensure_booking_assignment(
        company_id=company_id, booking=b, driver_id=DRIVER_ID
    )
    db.session.commit()
    try:
