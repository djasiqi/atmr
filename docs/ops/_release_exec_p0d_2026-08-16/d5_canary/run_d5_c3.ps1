# D5 C3 canary - transient mission loss (binary 128+)
# Inject via deep link lirie://canary/d5-c3-transient?hole_ms=1000
...
$timeline = Join-Path $OutDir "timeline_C3.txt"
$csv = Join-Path $OutDir "samples_C3.csv"
$logcatFile = Join-Path $OutDir "logcat_C3_continuous.txt"
...
if ($vc -lt $MinVersionCode) { TLog "ABORT need versionCode >= $MinVersionCode for C3 inject"; exit 2 }
...
function Get-NativeDelta {
  $tmp = Join-Path $OutDir "_nat_tmp_C3.txt"
  & $adb -s $d logcat -d -t 5000 2>$null |
    Select-String -Pattern "Unregistering|Finished task 'background-location-task'|Could not find a location task|D5-C3|transient_loss|inject_hole" |
  ...
  $unreg = @(Select-String -Path $tmp -Pattern "Unregistering 'background-location-task'|TaskService.*Unregistering" -EA SilentlyContinue).Count
  $c3 = @(Select-String -Path $tmp -Pattern "D5-C3" -EA SilentlyContinue).Count
  return [pscustomobject]@{ Finished = $fin; Unregister = $unreg; C3Marks = $c3 }
}
...
print('N', int(n or 0))
"@
  $pyFile = Join-Path $OutDir "_loc_snap_C3.py"
  [IO.File]::WriteAllText($pyFile, ($py -replace "`r`n", "`n"))
  scp -o BatchMode=yes -o ConnectTimeout=8 $pyFile "${ssh}:/tmp/d5_c3_snap.py" 2>$null | Out-Null
  $locOut = ssh -o BatchMode=yes -o ConnectTimeout=12 $ssh "docker cp /tmp/d5_c3_snap.py atmr-backend-1:/tmp/d5_c3_snap.py && docker exec atmr-backend-1 python /tmp/d5_c3_snap.py 2>/dev/null" 2>&1
...
function Sample([string]$phase, [int]$elapsed) {
  ...
  $row = "$ts,$phase,$elapsed,$($svc.FgCount),$($svc.Binds),$($svc.IsForeground),$($svc.StartRequested),$($nat.Finished),$($nat.Unregister),$($pl.PutN),$($pl.LocN),c3=$($nat.C3Marks)"
  [IO.File]::AppendAllText($csv, $row + [Environment]::NewLine)
  TLog "SAMPLE phase=$phase t=+${elapsed}s fg=$($svc.FgCount) binds=$($svc.Binds) isFg=$($svc.IsForeground) startReq=$($svc.StartRequested) finished=$($nat.Finished) unreg=$($nat.Unregister) c3marks=$($nat.C3Marks) PUT=$($pl.PutN) LOC=$($pl.LocN)"
  return $svc
}
...
$errLog = Join-Path $OutDir "logcat_C3_stderr.txt"
...
TLog "BACKEND=OBSERVATION_ONLY DRIVER=$DriverId HoleMs=$HoleMs"
...
TLog "INJECT_DEEPLINK hole_ms=$HoleMs"
$deeplink = "lirie://canary/d5-c3-transient?hole_ms=$HoleMs"
