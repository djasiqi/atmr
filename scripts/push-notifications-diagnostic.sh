...
REC="$OUT_DIR/recommendation.txt"
{
  ...
  echo ""
  echo "=== STOP GATE P0 — canaux Android attendus ==="
  echo "Payload: channelId=mission_updates (assignation), data.type=booking_assigned"
  echo "Aucun channelId missions ou missions_v2 en production."
  echo "ADB canaux: adb shell cmd notification channels ch.liri.operations"
  echo "Attendu: mission_updates, urgent, chat, default"
  echo "Chauffeurs exclus STOP GATE sans token actif: 6858, 7755"
} >"$REC"
