#!/bin/bash
# Script de diagnostic pour la connexion Backend → Worker RL

echo "🔍 Diagnostic de la connexion Backend → Worker RL"
echo "=================================================="
echo ""
...
echo "📡 Connexion au serveur: $SERVER_USER@$SERVER_IP"
echo ""
...
echo "   - Vérifier les réseaux: docker network ls"
echo "   - Connecter backend au réseau RL: docker network connect atmr-rl-network atmr-backend"
echo "   - Redémarrer le backend: cd /srv/atmr && docker compose -f docker-compose.production.yml restart backend"
echo "   - Vérifier les logs worker RL: docker logs atmr-rl-worker --tail 100"
